...
***************************************************************************/
	...
	//--------------------------------------------------------------------
			...
			$option = get_option('projectTheme_nr_max_of_images');
		...
		ProjectTheme_insert_pages('ProjectTheme_provider_search_page_id', 					'Freelancer Search', 		'[project_theme_provider_search]' );

		//-------------------

		ProjectTheme_insert_pages('ProjectTheme_post_new_page_id', 					'Post New', 				'[project_theme_post_new]' );
		ProjectTheme_insert_pages_account('ProjectTheme_my_account_page_id', 				'My Account', 				'[project_theme_my_account_home]' );
		ProjectTheme_insert_pages_account('ProjectTheme_my_account_personal_info_id', 		'Personal Information', 	'[project_theme_my_account_personal_info]', 	get_option('ProjectTheme_my_account_page_id') );
		ProjectTheme_insert_pages_account('ProjectTheme_my_account_payments_id', 			'Finances', 				'[project_theme_my_account_payments]', 			get_option('ProjectTheme_my_account_page_id') );
		ProjectTheme_insert_pages_account('ProjectTheme_my_account_private_messages_id', 	'Private Messages', 		'[project_theme_my_account_private_messages]', 	get_option('ProjectTheme_my_account_page_id') );
		ProjectTheme_insert_pages_account('ProjectTheme_my_account_feedback_id', 			'Feedback/Reviews', 		'[project_theme_my_account_feedback]', 			get_option('ProjectTheme_my_account_page_id') );
	...
	$opt = get_option('pt_hdb3c8f71c85206e661d7538de50fxv');

	if(empty($opt) or isset($_GET['sitemile_reset_theme']))
	{
			update_option('pt_hdb3c8f71c85206e661d7538de50fxv','ya');
			global $wpdb;


			$ss = "ALTER TABLE `".$wpdb->prefix."project_payment_transactions` ADD  `pid_related` VARCHAR( 255 ) NOT NULL DEFAULT '0' ;";
			$wpdb->query($ss);


			$ss = " CREATE TABLE `".$wpdb->prefix."project_marketplace_payments` (
				 `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
				 `fromid` BIGINT NOT NULL DEFAULT '0' ,
				 ...
				 `field1` VARCHAR( 255 ) NOT NULL DEFAULT '0',
				 `field2` VARCHAR( 255 ) NOT NULL DEFAULT '0',
				 `field3` VARCHAR( 255 ) NOT NULL DEFAULT '0',
				 `field4` VARCHAR( 255 ) NOT NULL DEFAULT '0',
				 `field5` VARCHAR( 255 ) NOT NULL DEFAULT '0'
				 ) ENGINE = MYISAM ";
			$wpdb->query($ss);


			// on the connected gateways helps keep track of the commission for site admin

			$ss = " CREATE TABLE `".$wpdb->prefix."project_marketplace_payments_commissions` (
				 `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
				 `oid` BIGINT NOT NULL  DEFAULT '0',
				 `amount` DOUBLE NOT NULL DEFAULT '0' ,
				 `datemade` BIGINT NOT NULL DEFAULT '0' ,
			...
			//on connected gateways, keeps track of amounts the freelancers make

			$ss = " CREATE TABLE `".$wpdb->prefix."project_marketplace_payments_freelancers` (
				 `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
				 `oid` BIGINT NOT NULL  DEFAULT '0',
				 `uid` BIGINT NOT NULL  DEFAULT '0',
				 `amount` DOUBLE NOT NULL DEFAULT '0' ,
 ...
 //$wpdb->query($ss);
			...
			$ss = "ALTER TABLE `".$wpdb->prefix."project_pm_threads` ADD  `user1_last_type` BIGINT NOT NULL DEFAULT '0';";
			$wpdb->query($ss);

			$ss = "ALTER TABLE `".$wpdb->prefix."project_pm_threads` ADD  `user2_last_type` BIGINT NOT NULL DEFAULT '0';";
			$wpdb->query($ss);


			ProjectTheme_insert_pages_account('ProjectTheme_my_account_buyer_area', 				"Buyer Area", 				'[project_theme_my_account_buyer_area]', 				get_option('ProjectTheme_my_account_page_id') );
			ProjectTheme_insert_pages_account('ProjectTheme_my_account_freelancer_area', 		"Freelancer Area", 		'[project_theme_my_account_freelancer_area]', 	get_option('ProjectTheme_my_account_page_id') );
			ProjectTheme_insert_pages_account('ProjectTheme_my_account_workspaces_id', 			'Workspaces', 				'[project_theme_my_account_workspaces]', 				get_option('ProjectTheme_my_account_page_id') );
			// order status: 0 - open, 1 - freelancer delivered, 2 - buyer accepted(success), 3 - order cancelled
			$ss = "CREATE TABLE `".$wpdb->prefix."project_orders` (
					`id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
					`buyer` BIGINT NOT NULL DEFAULT '0',
					`freelancer` BIGINT NOT NULL DEFAULT '0',
					`pid` BIGINT NOT NULL DEFAULT '0',
					`datemade` BIGINT NOT NULL DEFAULT '0',

					`done_freelancer` TINYINT NOT NULL DEFAULT '0',
					`done_buyer` TINYINT NOT NULL DEFAULT '0',
					`order_status` TINYINT NOT NULL DEFAULT '0',

					`marked_done_freelancer` BIGINT NOT NULL DEFAULT '0',
			...
			$ss = "ALTER TABLE `".$wpdb->prefix."project_orders` ADD  `completion_date` BIGINT NOT NULL DEFAULT '0';";
			$wpdb->query($ss);

				//------- recheck everything --- transform things---- do not touch this after update ----
				$option_once = get_option('option_oncev411');
				if(empty($option_once))
				{
							update_option('option_oncev411','on');
							global $wpdb;
							$s = "select * from ".$wpdb->prefix."project_bids where winner='1'";
							...
							foreach($r as $row)
							{
									...
									$mark_coder_delivered = get_post_meta($row->pid,'mark_coder_delivered',true); if(empty($mark_coder_delivered)) $mark_coder_delivered = 0;
									$mark_coder_delivered_date = get_post_meta($row->pid,'mark_coder_delivered_date',true); if(empty($mark_coder_delivered_date)) $mark_coder_delivered_date = 0;

									$mark_seller_accepted = get_post_meta($row->pid,'mark_seller_accepted',true); if(empty($mark_seller_accepted)) $mark_seller_accepted = 0;
									$mark_seller_accepted_date = get_post_meta($row->pid,'mark_seller_accepted_date',true); if(empty($mark_seller_accepted_date)) $mark_seller_accepted_date = 0;

									$order_total_amount = $order_net_amount = $row->bid;

									$sts = 0;
									if($mark_coder_delivered == 1) $sts = 1;
									if($mark_seller_accepted == 1) $sts = 2;

									$completion_date = $row->date_choosen + 24*3600*$row->days_done;
									...
									values('$completion_date','$buyer','{$row->uid}', '{$row->pid}', '{$row->date_choosen}', '$sts' , '$mark_coder_delivered', '$mark_coder_delivered_date', '$mark_seller_accepted', '$mark_seller_accepted_date', '$order_net_amount', '$order_total_amount')";
									$wpdb->query($s1);
							}
				...
				$ss = " CREATE TABLE `".$wpdb->prefix."project_escrows` (
						`id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
				 ...
				 $wpdb->query("ALTER TABLE `".$wpdb->prefix."project_escrows` ADD `method` VARCHAR( 255 ) NOT NULL ;");

				 //---------

				 $ss = "ALTER TABLE `".$wpdb->prefix."project_orders` ADD  `cancelled_date` BIGINT NOT NULL DEFAULT '0';";
				 $wpdb->query($ss);


				 global $wpdb;

				 $ss = " CREATE TABLE `".$wpdb->prefix."project_bills_site` (
						 `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY ,
						 `uid` BIGINT NOT NULL ,
						 `pid` BIGINT NOT NULL ,
						 `datemade` BIGINT NOT NULL ,
