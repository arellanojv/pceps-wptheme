		...
		?>
		 ...
		 <div class="page_heading_me_project pt_template_page_1 " id="pt_template_page_1">
		 													...
		 													<div class="main-pg-title col-xs-12 col-sm-12 col-md-12 col-lg-12" id="x_templ_1_pt">
		 															<div class="mm_inn mm_inn21"><?php printf(__("Registration Complete - %s",'ProjectTheme'), get_bloginfo('name')); ?>  </div>
		 								</div></div></div></div>
											...
											<div class="account-main-area-1 col-xs-12 col-sm-12 col-md-12 col-lg-12">
																<div class="card">
																	<div class="p-4">
																		...
																		$projectTheme_admin_approves_each_user 	= get_option('projectTheme_admin_approves_each_user');

																		if($projectTheme_admin_approves_each_user == "yes")
																		{
																				echo '<p>'; _e('Your account is not active yet. You need to confirm the activation link sent to your account.','ProjectTheme');
																				echo '</p>';
																		}
																		 ?>
							...
							<?php _e("Please check your <strong>Spam Folder</strong> if your account information does not appear within 5 minutes.",'ProjectTheme'); ?>
                            </p>
...
<div class="container mt-4">
		<div id="main" class="wrapper-register">
				  ...
				  <div class="my_box4 card"><div class="padd25">
				...
				<h3 class="h3-name"><?php printf(__("Register",'ProjectTheme')); ?></h3>
                          ...
                          <form method="post" id="registerform" action="<?php echo esc_url( get_site_url() . '/wp-login.php?action=register&wpe-login=true' ); ?>">
						  ...
						  <input type="hidden" name="action" value="register" />
							...
							<input type="text" class="form-control" name="user_login" id="user_login" size="30" maxlength="20" placeholder="<?php _e('Your username','ProjectTheme') ?>" value="<?php echo esc_html($user_login); ?>" />
							</p>

							<p>

							<input type="text" class="form-control" name="user_email" id="user_email" size="30" maxlength="100" placeholder="<?php _e('Your email address','ProjectTheme') ?>" value="<?php echo esc_html($user_email); ?>" />
							</p>


							<p>

							<input type="password" class="form-control" name="password" id="password" size="30" maxlength="100"  placeholder="<?php _e('Your password','ProjectTheme') ?>"  />
							</p>


							<p>

							<input type="password" class="form-control" name="cpassword" id="cpassword" size="30" maxlength="100"  placeholder="<?php _e('Your password confirmation','ProjectTheme') ?>" />
							</p>
...
function Project_register_new_user_sitemile( $user_login, $user_email ) {
	...
	if ( $sanitized_user_login == '' ) {
		$errors->add( 'empty_username', __( 'Please enter a username.', 'ProjectTheme' ) );
	} elseif ( ! validate_username( $user_login ) ) {
		$errors->add( 'invalid_username', __( 'This username is invalid because it uses illegal characters. Please enter a valid username.', 'ProjectTheme' ) );
		$sanitized_user_login = '';
	} elseif ( username_exists( $sanitized_user_login ) ) {
		$errors->add( 'username_exists', __( 'This username is already registered, please choose another one.', 'ProjectTheme' ) );
	}

	// Check the e-mail address
	if ( $user_email == '' ) {
		$errors->add( 'empty_email', __( 'Please type your e-mail address.', 'ProjectTheme' ) );
	} elseif ( ! is_email( $user_email ) ) {
		$errors->add( 'invalid_email', __( 'The email address isn&#8217;t correct.', 'ProjectTheme' ) );
		$user_email = '';
	} elseif ( email_exists( $user_email ) ) {
		$errors->add( 'email_exists', __( 'This email is already registered, please choose another one.', 'ProjectTheme' ) );
	}


	if ( strlen($password) < 5 ) {
		$errors->add( 'pass', __( 'Please type a password with at least 5 characters.', 'ProjectTheme' ) );
	}
	else if ( $password!= $cpassword )
		{
		$errors->add( 'pass', __( 'Your password doesnt match the confirmation.', 'ProjectTheme' ) );
	}
	...
	do_action('projecttheme_right_on_register', $user_id);
