...
<div class="page-header">
					<h1 class="page-title">
						<?php echo sprintf(__('Private Messages','ProjectTheme')  ) ?>
					</h1>
				</div></div></div>
						...
						<tr class="<?php echo $see; ?>">
								<td>
										<div class="ckbox ckbox-theme">
												<input id="checkbox1" type="checkbox" name="messages_id[]" value="<?php echo $row->id ?>" class="mail-checkbox">
...
<div class="w-100 mb-3"><a  href="<?php echo get_permalink( get_option('ProjectTheme_my_account_private_messages_id')  ) ?>" class="btn btn-sm btn-primary"><?php _e('Return to inbox','ProjectTheme') ?></a></div>
            	...
            	<div class="card p-3">
							...
							<?php
																	...
																	if(isset($_POST['send_a']))
																	{
																		...
																		if(count($rr) == 0)
																		{
																			...
																			//echo $s;
																			do_action('projettheme_on_receiving_a_message', $tm,  $myuid, $uids);
							...
							<?php
										...
										$s = "select * from ".$wpdb->prefix."project_pm where threadid='{$row->id}' order by id asc";
										$r = $wpdb->get_results($s);
										...
										foreach($r as $row1)
										{
													if($row1->user == $current_user->ID and $row1->approved == 1)
													{
													...
													if($row1->initiator == $current_user->ID)
													{
															...
															echo '<br/>'.sprintf(__('File Attached: %s','ProjectTheme') , '<a href="'.wp_get_attachment_url($row1->file_attached).'">'.wp_get_attachment_url($row1->file_attached)."</a>") ;
																													...
																													echo '<br/><span class="m1emm1">' . sprintf(__('Written on: %s','ProjectTheme'), date_i18n('d-M-Y H:i:s', $row1->datemade)) . '</span>';
													...
													elseif($row1->approved == 1) {
															...
															echo '<br/>'.sprintf(__('File Attached: %s','ProjectTheme') , '<a href="'.wp_get_attachment_url($row1->file_attached).'">'.wp_get_attachment_url($row1->file_attached)."</a>") ;
																													...
																													echo '<br/><br/><span class="m1emm1">' . sprintf(__('Written on: %s','ProjectTheme'), date_i18n('d-M-Y H:i:s', $row1->datemade)) . '</span>';
        ...
        <?php
			...
			if(isset($_POST['send_a']))
			{
				...
				if(count($rr) == 0)
				{
					...
					if($ProjectTheme_moderate_private_messages == false)
						ProjectTheme_send_email_on_priv_mess_received($myuid, $uid_send);
					else
					{
						//send message to admin to moderate
