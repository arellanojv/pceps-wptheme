	...
	//---------------------------------------------------------
	 ...
	 wp_set_object_terms($pid, $slg_arr,'project_skill');
		...
		if($type == "input_box")
		{
					...
					{
						$projectOK = 0;
						$MYerror['project_price'] 	= __('You cannot have the project price negative','ProjectTheme');
						$class_errors['project_price']		= 'error_class_post_new';
				}
		}
		...
		$exp_date = str_replace('/', '-', $_POST['ending1']);
		$exp_date = strtotime($exp_date);



		if($_POST['featured'] == 1) $projectTheme_project_period = get_option('projectTheme_project_period_featured');
		if(empty($projectTheme_project_period)) $projectTheme_project_period = 15;

		$ending = current_time('timestamp',0) + $projectTheme_project_period*3600*24;
		...
		update_post_meta($pid, "deadline", 		$exp_date); // ending date for the project
...
//************************ STEP 2 SUBMIT *********************************
	...
	if(isset($_POST['project_submit2']))
	{
		...
		if($_POST['featured'] == 1){
			...
			$projectTheme_project_period = get_option('projectTheme_project_period_featured');
			if(empty($projectTheme_project_period)) $projectTheme_project_period = 15;
