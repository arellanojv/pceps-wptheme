...
function projectTheme_options()
{
	...
	if(isset($_POST['my_submit']))
	{
		...
		$ProjectTheme_enable_map_proj_page		= trim($_POST['ProjectTheme_enable_map_proj_page']);
		...
		$ProjectTheme_allow_before_placing_a_bid			= $_POST['ProjectTheme_allow_before_placing_a_bid'];
		$ProjectTheme_enable_project_message_board			= $_POST['ProjectTheme_enable_project_message_board'];

		//**********************************************************************************************

						update_option('ProjectTheme_enable_project_message_board', 	$ProjectTheme_enable_project_message_board);
										update_option('projectTheme_admin_approves_each_user', 	$_POST['projectTheme_admin_approves_each_user']);
		...
		update_option('ProjectTheme_allow_before_placing_a_bid', $ProjectTheme_allow_before_placing_a_bid);
  ...
  <form method="post" action="">
        ...
        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_show_subcats_enbl'); ?></td>
				...
				<tr>
        <td valign=top width="22"><?php echo $menu_admin_project_theme_bull; ?></td>
        <td >Allow bidders to contact project owner before placing a bid:</td>
        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_allow_before_placing_a_bid'); ?></td>
        </tr>


				<tr>
        <td valign=top width="22"><?php echo $menu_admin_project_theme_bull; ?></td>
        <td >Enable project message board:</td>
        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_enable_project_message_board'); ?></td>
        </tr>
        ...
        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_enable_project_location'); ?></td>
				...
				<tr>
        <td valign=top width="22"><?php echo $menu_admin_project_theme_bull; ?></td>
        <td >Enable Map in Project Page:</td>
        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_enable_map_proj_page'); ?></td>
        </tr>
        ...
        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'projectTheme_admin_approves_each_project'); ?></td>
				...
				<tr>
				<td valign=top width="22"><?php echo $menu_admin_project_theme_bull; ?></td>
				<td >Enable user activation link in email:</td>
				<td><?php echo ProjectTheme_get_option_drop_down($arr, 'projectTheme_admin_approves_each_user'); ?></td>
				</tr>
...
//----------------------------------------------------------------------
	...
	if(isset($_GET['release']))
	{
		...
		$id = $_GET['release'];
						...
						$s = "select * from ".$wpdb->prefix."project_escrows where id='$id'";
						...
						//	sitemile_send_email($usr->user_email, $subject , $message);
							...
							$tm = current_time('timestamp',0);
							$s = "update ".$wpdb->prefix."project_escrows set released='1', releasedate='$tm' where
							id='$id'";
							$wpdb->query($s);
	...
	if(isset($_GET['close']))
	{
		...
		$s = "delete from ".$wpdb->prefix."project_escrows where	id='$id'";
		$wpdb->query($s);
  ...
  <div id="tabs1" style="display: block; ">
    <?php
		...
		$s = "select * from ".$wpdb->prefix."project_escrows where released='0' order by id desc";
		$r = $wpdb->get_results($s);
    ...
    <?php endif; ?>
		...
		$s = "select * from ".$wpdb->prefix."project_escrows where released='1' order by id desc";
		$r = $wpdb->get_results($s);
...
function projectTheme_theme_withdrawals()
{
	...
	if(isset($_GET['tid']))
	{
		...
		if($row->done == 0)
		{
			...
			$reason = sprintf(__('Withdraw to %s to email: %s','ProjectTheme'), $row->methods ,$row->payeremail);
...
function projectTheme_layout_settings()
{
	...
	echo '<h2 class="my_title_class_sitemile">'.$ttl_of_stuff.'</h2>';
		...
		if(isset($_POST['ProjectTheme_save4']))
		{
			...
			update_option('ProjectTheme_color_for_footer', 				trim($_POST['ProjectTheme_color_for_footer']));
				update_option('ProjectTheme_color_for_post_box', 				trim($_POST['ProjectTheme_color_for_post_box']));
					update_option('ProjectThemePrimaryColor', 				trim($_POST['ProjectThemePrimaryColor']));
						update_option('ProjectThemePrimaryColor2', 				trim($_POST['ProjectThemePrimaryColor2']));
							update_option('projecttheme_font', 				trim($_POST['projecttheme_font']));
		...
		if(isset($_POST['ProjectTheme_save3']))
		{
			...
			update_option('ProjectTheme_bottom_bar_bg_col', 				stripslashes(trim($_POST['ProjectTheme_bottom_bar_bg_col'])));
			update_option('ProjectTheme_bottom_bar_text_col', 				stripslashes(trim($_POST['ProjectTheme_bottom_bar_text_col'])));
		...
		if(isset($_POST['ProjectTheme_save_hm']))
		{
			...
			update_option('ProjectTheme_enable_searchbar', 			 (trim($_POST['ProjectTheme_enable_searchbar'])));
			update_option('ProjectTheme_main_blur_margin_top', 			 (trim($_POST['ProjectTheme_main_blur_margin_top'])));


			update_option('ProjectTheme_text_size_second_tag', 			 (trim($_POST['ProjectTheme_text_size_second_tag'])));
			update_option('ProjectTheme_text_size_main_tag', 			 	(trim($_POST['ProjectTheme_text_size_main_tag'])));
			update_option('ProjectTheme_color_main_tag', 			 	(trim($_POST['ProjectTheme_color_main_tag'])));
			update_option('ProjectTheme_color_second_tag', 			 	(trim($_POST['ProjectTheme_color_second_tag'])));


			update_option('ProjectTheme_align_elements_of_slider', 				stripslashes(trim($_POST['ProjectTheme_align_elements_of_slider'])));
		 ...
		 do_action('projecttheme_layoutsettings_save');
		//-----------------------------------------
...
?>
	    ...
	    <div id="usual2" class="usual">
          <ul>
            ...
            <li><a href="#tabs1"><?php _e('Header','ProjectTheme'); ?></a></li>
            <li><a href="#tabs_home" <?php echo $_GET['activate_tab'] == "tabs_home" ? "class='selected'" : ""; ?>><?php _e('HomePage Slider','ProjectTheme'); ?></a></li>
            <li><a href="#tabs6"><?php _e('Page Templates','ProjectTheme'); ?></a></li>
						<li><a href="#tabs3"><?php _e('Footer','ProjectTheme'); ?></a></li>
            <li><a href="#tabs4"><?php _e('Change Colors & Styles','ProjectTheme'); ?></a></li>
						<?php do_action('projecttheme_layoutsettings_tabs') ?>
          </ul>


					<?php do_action('projecttheme_layoutsettings_divs') ?>
          ...
          <div id="tabs_home">
           <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=layout-settings&active_tab=tabs_home">
            <table width="100%" class="sitemile-table">
							...
							<tr>
			        <td width="200"><?php _e('Margin Top Content:','ProjectTheme'); ?></td>
			        <td><input type="text" size="50" name="ProjectTheme_main_blur_margin_top" value="<?php echo get_option('ProjectTheme_main_blur_margin_top'); ?>" /> px
					</td>
			        </tr>
							...
							<td ><?php _e('Align slider:','ProjectTheme'); ?></td>
							<td>
							<?php echo ProjectTheme_get_option_drop_down($arr2, 'ProjectTheme_align_elements_of_slider'); ?>
							</td>
							</tr>
   <tr><td colspan="2">&nbsp;</td></tr>
			        ...
			        <td><input type="text" size="50" name="ProjectTheme_main_tagline" value="<?php echo stripslashes(get_option('ProjectTheme_main_tagline')); ?>" />
							</td>
			        </tr>


							<tr>
			        <td width="200"><?php _e('Font size (main tag line):','ProjectTheme'); ?></td>
			        <td><input type="text" size="15" name="ProjectTheme_text_size_main_tag" value="<?php echo stripslashes(get_option('ProjectTheme_text_size_main_tag')); ?>" />px
							</td>
			        </tr>


							<tr>
			        <td width="200"><?php _e('Color (main tag line):','ProjectTheme'); ?></td>
			        <td><input type="text" size="15" id='ProjectTheme_color_main_tag' name="ProjectTheme_color_main_tag" value="<?php echo stripslashes(get_option('ProjectTheme_color_main_tag')); ?>" />
							</td>
			        </tr>
   ...
   <tr><td colspan="2">&nbsp;</td></tr>
        	...
        	<td><input type="text" size="50" name="ProjectTheme_sec_tagline" value="<?php echo stripslashes(get_option('ProjectTheme_sec_tagline')); ?>" />
					</td>
        </tr>
				...
				<tr>
				<td width="200"><?php _e('Font size (second tag line):','ProjectTheme'); ?></td>
				<td><input type="text" size="15" name="ProjectTheme_text_size_second_tag" value="<?php echo get_option('ProjectTheme_text_size_second_tag'); ?>" />px
				</td>
				</tr>

				<tr>
				<td width="200"><?php _e('Color (secondary tag line):','ProjectTheme'); ?></td>
				<td><input type="text" size="15" id='ProjectTheme_color_second_tag' name="ProjectTheme_color_second_tag" value="<?php echo get_option('ProjectTheme_color_second_tag'); ?>" />
				</td>
				</tr>
        ...
        <td><input type="text" size="40" name="ProjectTheme_button2_link" value="<?php echo get_option('ProjectTheme_button2_link'); ?>" />
				...
				<tr>
        <td width="200"><?php _e('Enable Search Bar:','ProjectTheme'); ?></td>
        <td><input type="checkbox" size="40" name="ProjectTheme_enable_searchbar" value="1" <?php $cc = get_option('ProjectTheme_enable_searchbar'); if($cc == 1) echo 'checked="checked"'; ?>  />
		</td>
        </tr>
          ...
          <div id="tabs4">
           <form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=layout-settings&active_tab=tabs4">
            <table width="100%" class="sitemile-table">
							...
							<td ><?php _e('Choose Font:','ProjectTheme'); ?></td>
							<td>
							<select name="projecttheme_font">
											...
											$projecttheme_font = get_option('projecttheme_font');
													...
													foreach($google_fonts as $key => $value)
													{
																?>
																					...
																					<option <?php echo $key == $projecttheme_font ? "selected='selected'" : "" ?> value="<?php echo $key ?>"><?php echo $key ?></option>
							...
							<tr>
										<td ><?php _e('Font:','ProjectTheme'); ?></td>
										<td>
										<?php echo ProjectTheme_get_option_drop_down($arr2, 'ProjectTheme_align_elements_of_slider'); ?>
										</td>
							</tr>
...
jQuery(document).ready(function()
{
					...
					jQuery('#colorpickerField1, #colorpickerField2, #colorpickerField3, #ProjectTheme_color_for_post_box, #ProjectThemePrimaryColor, #ProjectThemePrimaryColor2, #ProjectTheme_color_main_tag, #ProjectTheme_color_second_tag').ColorPicker({
        ...
        <td><input type="text" id="ProjectTheme_color_for_post_box" name="ProjectTheme_color_for_post_box" value="<?php echo get_option('ProjectTheme_color_for_post_box'); ?>" />
				...
				<tr>
				<td width="200"><?php _e('Primary Color:','ProjectTheme'); ?></td>
				<td><input type="text" id="ProjectThemePrimaryColor" name="ProjectThemePrimaryColor" value="<?php echo get_option('ProjectThemePrimaryColor'); ?>" />
		</td>
				</tr>


				<tr>
				<td width="200"><?php _e('Primary Color (hover):','ProjectTheme'); ?></td>
				<td><input type="text" id="ProjectThemePrimaryColor2" name="ProjectThemePrimaryColor2" value="<?php echo get_option('ProjectThemePrimaryColor2'); ?>" />
		</td>
				</tr>
             ...
             <tr>
					...
					<form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=layout-settings&active_tab=tabs6">
						<table width="100%" class="sitemile-table">
							...
							<tr>
							<td valign=top width="22"><?php ProjectTheme_theme_bullet(__('Eg: 35','ProjectTheme')); ?></td>
							<td ><?php _e('Page Header Title Margin Top/Bottom:','ProjectTheme'); ?></td>
							<td>
        ...
        $('.wpclpick').wpColorPicker();
            ...
            <table width="100%" class="sitemile-table">
							...
							<tr>
							                    ...
							                    <td valign="top" ><?php _e('Text/Code Copyright (left side):','ProjectTheme'); ?></td>
							                    <td><textarea cols="65" rows="4" name="ProjectTheme_left_side_footer"><?php echo stripslashes(get_option('ProjectTheme_left_side_footer')); ?></textarea></td>
																	...
																	<tr>
																						<td valign=top width="22"><?php ProjectTheme_theme_bullet(__('Background color of the bottom most bar.','ProjectTheme')); ?></td>
																						<td valign="top" ><?php _e('Bottom footer bar bg color:','ProjectTheme'); ?></td>
																						<td>
																							<input type="text" id="ProjectTheme_bottom_bar_bg_col" size="35" class="wpclpick" name="ProjectTheme_bottom_bar_bg_col" value="<?php echo stripslashes(get_option('ProjectTheme_bottom_bar_bg_col')); ?>" />
																		 </td>
																						</tr>


																						<tr>
																											<td valign=top width="22"><?php ProjectTheme_theme_bullet(__('Text color of the bottom most bar.','ProjectTheme')); ?></td>
																											<td valign="top" ><?php _e('Bottom footer bar text color:','ProjectTheme'); ?></td>
																											<td>
																												<input type="text" id="ProjectTheme_bottom_bar_text_col" size="35" class="wpclpick" name="ProjectTheme_bottom_bar_text_col" value="<?php echo stripslashes(get_option('ProjectTheme_bottom_bar_text_col')); ?>" />
																							 </td>
																											</tr>
...
if(isset($_POST['ProjectTheme_deposit_money_wallet_email_save']))
{
	update_option('ProjectTheme_deposit_money_wallet_email_enable', 		trim($_POST['ProjectTheme_deposit_money_wallet_email_enable']));
	update_option('ProjectTheme_deposit_money_wallet_email_subject', 	trim($_POST['ProjectTheme_deposit_money_wallet_email_subject']));
	update_option('ProjectTheme_deposit_money_wallet_email_message', 	trim($_POST['ProjectTheme_deposit_money_wallet_email_message']));
	...
	<div id="usual2" class="usual">
        <ul>
            ...
            <li><a href="#deposit_money_wallet"><?php _e('Deposit Ewallet(user)','ProjectTheme'); ?></a></li>
...
<div id="deposit_money_wallet">

 <div class="spntxt_bo"><?php _e('This email will be received by the user when they have deposited money to their e-wallet.
Be aware, if you add html tags to this email you must have the allow HTML tags option set to yes.
Also at the bottom you can see a list of tags you can use in the email body.','ProjectTheme'); ?> </div>


<form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=email-settings&active_tab=deposit_money_wallet">
	<table width="100%" class="sitemile-table">
					...
					<td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
					<td ><?php _e('Enable this email:','ProjectTheme'); ?></td>
					<td><?php echo ProjectTheme_get_option_drop_down($arr, 'ProjectTheme_deposit_money_wallet_email_enable'); ?></td>
					</tr>

				<tr>
					<td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
					<td width="160"><?php _e('Email Subject:','ProjectTheme'); ?></td>
					<td><input type="text" size="90" name="ProjectTheme_deposit_money_wallet_email_subject" value="<?php echo stripslashes(get_option('ProjectTheme_deposit_money_wallet_email_subject')); ?>"/></td>
					</tr>
					...
					<td valign=top><?php ProjectTheme_theme_bullet(); ?></td>
					<td valign=top ><?php _e('Email Content:','ProjectTheme'); ?></td>
					<td><textarea cols="92" rows="10" name="ProjectTheme_deposit_money_wallet_email_message"><?php echo stripslashes(get_option('ProjectTheme_deposit_money_wallet_email_message')); ?></textarea></td>
					</tr>
					...
					<strong>##username##</strong> - <?php _e('Your username','ProjectTheme'); ?><br/>
					<strong>##site_login_url##</strong> - <?php _e('the link to your user login page','ProjectTheme'); ?><br/>
					<strong>##your_site_name##</strong> - <?php _e("your website's name","ProjectTheme"); ?><br/>
					<strong>##your_site_url##</strong> - <?php _e("your website's main address",'ProjectTheme'); ?><br/>

					<strong>##my_account_url##</strong> - <?php _e("your website's my account link",'ProjectTheme'); ?><br/>
					<strong>##method_deposit##</strong> - <?php _e("method of deposit",'ProjectTheme'); ?><br/>
					<strong>##amount##</strong> - <?php _e('deposited amount','ProjectTheme'); ?><br/>
					...
					<td ></td>
					<td ></td>
					<td><input type="submit"  class="button button-primary button-large" name="ProjectTheme_deposit_money_wallet_email_save" value="<?php _e('Save Options','ProjectTheme'); ?>"/></td>
					</tr>
...
function projectTheme_pricing_options()
{
	...
	$pay_model = array("ewallet_only" => __("E-wallet - Escrow - Withdrawals",'ProjectTheme'), "marketplace_gateways" => __("Marketplace Gateways - Direct Payment",'ProjectTheme')
	, "invoice_model_pay_outside" => __("Invoice Model - Pay outside",'ProjectTheme'), "pay_for_contact" => __("Pay for contact details",'ProjectTheme'));
	...
	if(isset($_POST['ProjectTheme_save1']))
	{
		...
		$ProjectTheme_enable_credits_wallet 		= trim($_POST['ProjectTheme_enable_credits_wallet']);
	...
	if(isset($_POST['ProjectTheme_save2']))
	{
		...
		$projectTheme_tax_fee_withdrawal					= trim($_POST['projectTheme_tax_fee_withdrawal']);
		$projectTheme_fee_after_paid_bidder					= trim($_POST['projectTheme_fee_after_paid_bidder']);
		$project_theme_contact_details_fee 		= trim($_POST['project_theme_contact_details_fee']);

		update_option('projectTheme_fee_after_paid_bidder', 					$_POST['projectTheme_fee_after_paid_bidder']);
		update_option('projectTheme_fee_service_fee', 					$_POST['projectTheme_fee_service_fee']);
		update_option('projectTheme_tax_fee_withdrawal', 					$projectTheme_tax_fee_withdrawal);
		...
		update_option('projectTheme_fee_after_paid', 			$projectTheme_fee_after_paid);
				...
				update_option('project_theme_min_withdraw', 			$project_theme_min_withdraw);
						update_option('project_theme_contact_details_fee', 			$project_theme_contact_details_fee);
						...
						<tr>
								...
								<td colspan=2>
										...
										<br/><br/>
																				...
																				<b>Invoice Model - Pay outside</b> - payment for the actual project is done outside of the website. Customer and freelancer are charged commission fee. When a user is not paying, you can suspend their account, maybe using a plugin.<br/> <br/>

												 	<b>Pay for Contact Details</b> - freelancer pays for getting the contact details
								</td>
						</tr>
  ...
  <div id="tabs2" style="display: none; ">
  	...
  	<form method="post" action="<?php echo get_admin_url(); ?>admin.php?page=pricing-settings&active_tab=tabs2">
            <table width="100%" class="sitemile-table">
                    ...
                    <td><input type="text" name="projectTheme_fee_after_paid" size="5" value="<?php echo  get_option('projectTheme_fee_after_paid'); ?>"  /> %<br/>
										If "pay outside" model is used, this fee applies to the project owner</td>
                    </tr>


										<tr>
                    <td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
                    <td ><?php _e('Commission Fee (out of the winning bid - for freelancer): ','ProjectTheme'); ?></td>
                    <td><input type="text" name="projectTheme_fee_after_paid_bidder" size="5" value="<?php echo  get_option('projectTheme_fee_after_paid_bidder'); ?>"  /> %<br/>
										This is used only on the "pay outside" model</td>
                    </tr>
										...
										<tr>
										<td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
										<td ><?php _e('Service Fee (on top of project price): ','ProjectTheme'); ?></td>
										<td><input type="text" name="projectTheme_fee_service_fee" size="5" value="<?php echo  get_option('projectTheme_fee_service_fee'); ?>"  /> %</td>
										</tr>


										<tr>
										<td valign=top width="22"><?php ProjectTheme_theme_bullet('This will be subtracted from the amount when users request withdrawal.'); ?></td>
										<td ><?php _e('Tax/Fee On withdrawal: ','ProjectTheme'); ?></td>
										<td><input type="text" name="projectTheme_tax_fee_withdrawal" size="5" value="<?php echo  get_option('projectTheme_tax_fee_withdrawal'); ?>"  /> <?php echo projectTheme_get_currency() ?> </td>
										</tr>
                    ...
                    <td><input type="text" name="project_theme_min_withdraw" size="10" value="<?php echo  get_option('project_theme_min_withdraw'); ?>"  /> <?php echo projectTheme_currency(); ?></td>
										...
										<tr>
										<td valign=top width="22"><?php ProjectTheme_theme_bullet(); ?></td>
										<td width="240" ><?php _e('Fee for contact details:','ProjectTheme'); ?></td>
										<td><input type="text" name="project_theme_contact_details_fee" size="10" value="<?php echo  get_option('project_theme_contact_details_fee'); ?>"  /> <?php echo projectTheme_currency(); ?> (this fee is paid by freelancer to get the contact details of users)</td>
										</tr>
...
function projectTheme_cust_prcng()
{
	...
	$arr = array("yes" => "Yes", "no" => "No");
	$arr2 = array("categories" => "Based on categories", "budget" => "Based on budget");
	   ...
	   if(isset($_POST['my_submit2']))
	{
		...
		update_option('projectTheme_model_bidding_fees', $_POST['projectTheme_model_bidding_fees']);
		//---------------
		...
		for ($i=1 ;$i<10; $i++)
		{
					update_option('project_bid_fee_price_from' . $i, $_POST['project_bid_fee_price_from' . $i]);
					update_option('project_bid_fee_price_to' . $i, $_POST['project_bid_fee_price_to' . $i]);
					update_option('project_bid_fee_budget_' . $i, $_POST['project_bid_fee_budget_' . $i]);
  ...
  <div id="tabs1" style="display: block; ">
        ...
        <td><?php echo ProjectTheme_get_option_drop_down($arr, 'projectTheme_enable_custom_bidding'); ?></td>
				...
				<tr>
        <td width="220" >Bidding Fees Model:</td>
        <td><?php echo ProjectTheme_get_option_drop_down($arr2, 'projectTheme_model_bidding_fees'); ?></td>
        </tr>
 ...
 <?php echo ProjectTheme_project_clear_table(2); ?>
		...
		<tr>
	 <td width="220" ><strong>Set Fees for budgets:</strong></td>
	 <td></td>
	 </tr>
	 <?php echo ProjectTheme_project_clear_table(2); ?>
	 ...
	 <?php
				 ...
				 for ($i=1 ;$i<10; $i++)
				 {
					 ...
					 echo '<tr>';
					 echo '<td>
					 <input type="text" size="14" value="'.get_option('project_bid_fee_price_from' . $i).'" name="project_bid_fee_price_from'.$i.'" placeholder="Budget From ('.projecttheme_get_currency().')" /><br/>

					 <input type="text" size="14" value="'.get_option('project_bid_fee_price_to' . $i).'" name="project_bid_fee_price_to'.$i.'" placeholder="Budget To ('.projecttheme_get_currency().')" />

					 </td>';
					 echo '<td><input type="text" size="6" value="'.get_option('project_bid_fee_budget_'.$i).'"
					 name="project_bid_fee_budget_'.$i.'" placeholder="Fee" /> '.projectTheme_currency().'
					 </td>';
