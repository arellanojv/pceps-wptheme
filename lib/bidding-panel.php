...
<?php
  ...
  {
    $bidFeePaid = get_option('projectTheme_bid_fee_was_paid' . $uid . $pid);
    if(empty($bidFeePaid))
    {
        $is_it_allowed = false;
        $bid_fee_not_paid = 1;
	...
	if($is_it_allowed == false):
     ...
     if($bid_fee_not_paid == 1)
     {
          $fee = projecttheme_get_bidding_fee_based_on_budget($pid);
          $feeFormatted = projectTheme_get_show_price($fee);
       ?>
          <p><?php echo sprintf(__('In order to bid, you have to pay the bidding fee. Amount is: %s','ProjectTheme'), $feeFormatted); ?></p>
          ...
          if($ProjectTheme_paypal_enable == "yes")
          {
              ?>
                    ...
                    <a href="<?php echo get_site_url() ?>/?pay_bidding_fee_paypal=<?php echo $pid; ?>" class="btn btn-primary"><?php _e('Pay by PayPal','ProjectTheme') ?></a>
